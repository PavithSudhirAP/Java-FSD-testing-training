...
-- 1)
select o.id as order_id,c.phone_number from orders o
join customers c on o.customer_id=c.id
where o.product_id = 4;

-- 2)
select p.name as product_name,o.order_time from products p
join orders o on p.id = o.product_id
where p.name = 'Filter'
...
-- 3)
select p.name as product_name,p.price,o.order_time from orders o
